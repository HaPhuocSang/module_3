create database if not exists quan_ly_khu_furama;
use quan_ly_khu_furama;
...
(8, 2, 12, 2);
-- câu 2 
select * from nhan_vien n where (n.ho_ten like "H%" or n.ho_ten like "T%" or n.ho_ten like "K%") and char_length(n.ho_ten) <= 15; 
-- câu 3
select * from khach_hang k where (TIMESTAMPDIFF(YEAR, k.ngay_sinh, CURDATE()) between 18 and 50) and (k.dia_chi like "%Đà Nẵng" or k.dia_chi like "%Quảng Trị");
-- câu 4
select k.ma_khach_hang, k.ho_ten, count(hd.ma_khach_hang) as so_lan_dat_phong from khach_hang k 
...
-- câu 5
select k.ma_khach_hang, k.ho_ten, lk.ten_loai_khach, hd.ma_hop_dong, dv.ten_dich_vu, hd.ngay_lam_hop_dong, hd.ngay_ket_thuc, (coalesce(sum(dv.chi_phi_thue), 0)+(coalesce(sum(hdct.so_luong*dvdk.gia), 0))) as tong_tien
from khach_hang k
...
group by k.ma_khach_hang, k.ho_ten, lk.ten_loai_khach, hd.ma_hop_dong, dv.ten_dich_vu, hd.ngay_lam_hop_dong, hd.ngay_ket_thuc; 
-- câu 6
with ds_ma_dich_vu as (select dv.ma_dich_vu
from dich_vu dv 
...
where dv.ma_dich_vu not in(select * from ds_ma_dich_vu) 
order by ldv.ten_loai_dich_vu desc;
-- câu 7
with ds_ma_dich_vu_2021 as (select dv.ma_dich_vu
from dich_vu dv 
right join hop_dong hd on hd.ma_dich_vu = dv.ma_dich_vu
where year(hd.ngay_lam_hop_dong) = 2021),
ds_ma_dich_vu_2020 as (select dv.ma_dich_vu
from dich_vu dv 
right join hop_dong hd on hd.ma_dich_vu = dv.ma_dich_vu
